  ...
  def interpret(context: Context): Unit = {
    clazz match {
      case Some(cla) =>
        context get cla match {
          case RuntimeClass(cl) =>
            val result =
              JavaInterop.callMethod(context, cl, id, args.map(_ eval context))
      ...
      case None =>
        ...
        if (evaluatedArgs.head == null)
          throw new FimException("There's nothing here")
        val result = JavaInterop.callMethod(
          context,
          evaluatedArgs.head.toJava.getClass,
          id,
          evaluatedArgs
        )
        returnTo foreach { context.set(_, result) }
...
case class ConstructorCall(
  ...
  def interpret(context: Context): Unit = {
    context get clazz match {
      case RuntimeClass(cl) =>
        val result =
          JavaInterop.callConstructor(context, cl, args.map(_ eval context))
        returnTo foreach { context.set(_, result) }
  ...
  def interpret(context: Context): Unit = {
    ...
    (oc, field) match {
      case (RuntimeClass(cl), VariableValue(n)) =>
        JavaInterop.setField(context, Left(cl), Right(n), v)
      case (RuntimeClass(cl), NumberValue(n)) =>
        JavaInterop.setField(context, Left(cl), Left(n.toInt), v)
      case (RuntimeJavaObject(w), VariableValue(n)) =>
        JavaInterop.setField(context, Right(w), Right(n), v)
      case (RuntimeJavaObject(w), NumberValue(n)) =>
        JavaInterop.setField(context, Right(w), Left(n.toInt), v)
...
case class FieldRetrieval(objClass: String, field: Expr, otherVar: String)
  ...
  def interpret(context: Context): Unit = {
    ...
    val result = (oc, field) match {
      case (RuntimeClass(cl), VariableValue(n)) =>
        JavaInterop.getField(Left(cl), Right(n))
      case (RuntimeClass(cl), StringValue(n)) =>
        JavaInterop.getField(Left(cl), Right(n))
      case (RuntimeClass(cl), Concatenation(List(StringValue(n)))) =>
        JavaInterop.getField(Left(cl), Right(n))
      case (RuntimeClass(cl), NumberValue(n)) =>
        JavaInterop.getField(Left(cl), Left(n.toInt))
      case (RuntimeJavaObject(v), VariableValue(n)) =>
        JavaInterop.getField(Right(v), Right(n))
      case (RuntimeJavaObject(v), StringValue(n)) =>
        JavaInterop.getField(Right(v), Right(n))
      case (RuntimeJavaObject(v), Concatenation(List(StringValue(n)))) =>
        JavaInterop.getField(Right(v), Right(n))
      case (RuntimeJavaObject(v), NumberValue(n)) =>
        JavaInterop.getField(Right(v), Left(n.toInt))
    ...
    context.get(arrayVariable) match {
      case a: RuntimeArray =>
        index.eval(context) match {
          case RuntimeNumber(i) => {
            if (i <= 0) {
              throw new FimException(
                "Tried to write to a zero or a negative page of a book",
                index.pos
              )
            } else {
          ...
          case _ =>
            throw new FimException("This is not a page number", index.pos)
        }
      case _ => throw new FimException(arrayVariable + " is not a book")
    }
  }
}
case class ArrayRetrieval(
      ...
      case a: RuntimeArray =>
        index.eval(context) match {
          case RuntimeNumber(i) => {
            if (i <= 0) {
              throw new FimException(
                "Tried to read zero or a negative page of a book",
                index.pos
              )
            } else {
...
case class Increment(variable: String, value: Expr) extends Statement {
  def interpret(context: Context) : Unit = {
    (context.get(variable), value.eval(context)) match {
      ...
      case _ =>
        throw new FimException(
          variable + " is not somepony we can increment or decrement",
          pos
        )
    }
  }
