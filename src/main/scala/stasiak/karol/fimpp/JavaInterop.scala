...
object JavaInterop {
  def normalizeFromFim(str: String) =
    str.toLowerCase.replaceAll(" ", "").replaceAll("\'", "").replaceAll("-", "")
  ...
  def normalizedMatch(
    ...
    if (fromFimNormalized.charAt(0) == '?')
      "is" + fromFimNormalized.substring(
        1
      ) == fromJavaNormalized || "get" + fromFimNormalized.substring(
        1
      ) == fromJavaNormalized
  ...
  def callMethod(
      ...
      clazz: Class[_],
      methodName: String,
      methodParams: List[RuntimeValue]
  ): RuntimeValue = {
    ...
    clazz.getMethods.foreach { method =>
      ...
      if (
        ...
        && method.getParameterTypes.size == (if (isStatic(method.getModifiers))
                                               methodParams.size
                                             else methodParams.size - 1)
      ) {
        val score = (if (isStatic(method.getModifiers)) methodParams
                     else methodParams.tail)
          .zip(method.getParameterTypes)
          .map { case (rv, cl) =>
            rv.matches(cl)
          }
          .product * (if (isStatic(method.getModifiers)) 1
                      else methodParams.head.matches(clazz))
    ...
    if (best eq null)
      throw new FimException("Method ‘" + methodName + "’ not found")
    if (isStatic(best.getModifiers)) {
      val args = (methodParams zip best.getParameterTypes)
        .map { case (rv, cl) =>
      ...
      val args = (methodParams.tail zip best.getParameterTypes)
        .map { case (rv, cl) =>
          rv.convertTo(context, cl)
        }
        .toSeq
        .asInstanceOf[Seq[java.lang.Object]]
      RuntimeValue.fromJava(
        best.invoke(methodParams.head.convertTo(context, clazz), args: _*)
      )
    }
  }
  def callConstructor(
      context: Context,
      clazz: Class[_],
      constructorParams: List[RuntimeValue]
  ): RuntimeValue = {
    ...
    clazz.getConstructors.foreach { constructor =>
      if (
        ...
        ) && constructor.getParameterTypes.size == constructorParams.size
      ) {
        val score = constructorParams
          .zip(constructor.getParameterTypes)
          .map { case (rv, cl) =>
            rv.matches(cl)
          }
          .product
    ...
    val args = (constructorParams zip best.getParameterTypes)
      .map { case (rv, cl) =>
        rv.convertTo(context, cl)
      }
      .toSeq
  ...
  def getField(
      objClass: Either[Class[_], Any],
      field: Either[Int, String]
  ): RuntimeValue = RuntimeValue.fromJava(
    (objClass, field) match {
      case (Left(clazz), Left(i)) =>
        throw new FimException("Classes are not indexable")
  ...
  def loadClass(id: List[String]): Class[_] = {
    ...
    val allPossibleClasses =
      for (
        ...
        cn <- allPossibleShortClassNames(id.drop(packageLength))
      ) yield pack + "." + cn
    // println(allPossibleClasses)
    val classes = allPossibleClasses.filter { className =>
      try {
