...
/** Created with IntelliJ IDEA. User: karol Date: 06.10.12 Time: 12:57 To change
...
case class RuntimeBoolean(truth: Boolean) extends RuntimeValue {
  ...
  def convertTo(context: Context, clazz: Class[_]) =
    java.lang.Boolean.valueOf(truth)
  def toJava = truth
}
...
case class RuntimeBuiltin(function: List[RuntimeValue] => RuntimeValue)
...
case class RuntimeJavaObject(obj: Any) extends RuntimeValue {
  ...
  def matches(clazz: Class[_]) =
    if (clazz.isAssignableFrom(obj.getClass)) 1 else 0 // TODO
...
case class RuntimeClass(clazz: java.lang.Class[_]) extends RuntimeValue {
  ...
  def matches(clazz2: Class[_]) =
    if (clazz2.isAssignableFrom(classOf[Class[_]])) 1 else 0 // TODO
  def convertTo(context: Context, clazz2: Class[_]) = clazz // TODO
  def toJava = clazz
}

class Context private (
    map: mutable.Map[String, RuntimeValue],
