...
/** Created with IntelliJ IDEA. User: karol Date: 06.10.12 Time: 16:47 To change
...
object BuiltInFunctions {
  ...
  def dictionary(args: List[RuntimeValue]) = args match {
    case List(RuntimeList(l)) =>
      RuntimeArray(new ArrayBuffer[RuntimeValue]() ++ l)
  ...
  def first(args: List[RuntimeValue]) = args match {
    ...
    case _ =>
      throw new FimException(
        "Trying to get a first element from something that is neither list nor book"
      )
  }
  def letters(args: List[RuntimeValue]) = args match {
    case List(RuntimeString(l)) =>
      ...
      result
    case _ =>
      throw new FimException("You can extract letters from only one string")
  }
